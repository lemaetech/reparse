...
let test_lnum_cnum_1 () =
  let p =
    P.skip P.next *> P.map2 (fun lnum cnum -> (lnum, cnum)) P.lnum P.cnum in
  let r = P.parse ~track_lnum:true (input "hello world") p in
  Alcotest.(check (pair int int) "1/12" (1, 12) r)

let test_lnum_cnum_2 () =
  let p =
    P.skip P.next *> P.map2 (fun lnum cnum -> (lnum, cnum)) P.lnum P.cnum in
  let r = P.parse ~track_lnum:true (input "hello world\nsecond line\n") p in
...
let test_lnum_cnum_4 () =
  let p =
    P.skip P.next *> P.map2 (fun lnum cnum -> (lnum, cnum)) P.lnum P.cnum in
  let r = P.parse (input "hello world\nsecond line\naaaaa") p in
  Alcotest.(check (pair int int) "0/0" (0, 0) r) ;
  let p =
    P.skip P.next *> P.map2 (fun lnum cnum -> (lnum, cnum)) P.lnum P.cnum in
  let r =
    P.parse ~track_lnum:false (input "hello world\nsecond line\naaaaa") p in
  Alcotest.(check (pair int int) "0/0" (0, 0) r)
