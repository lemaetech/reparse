module Make_test (P : Test_parser.TEST_PARSER) = struct
  ...
  let unsafe_take_cstruct =
    ...
    let inp () = P.of_string "hello world" in
    let p2 =
      (P.unsafe_take_cstruct 6, P.unsafe_take_cstruct 5) <$$> fun a b ->
      Cstruct.append a b
    in
    Popper.(
      suite
  ...
  let unsafe_take_cstruct_ne =
    let p = P.unsafe_take_cstruct_ne 5 in
    let inp () = P.of_string "hello world" in
    let p2 =
      (P.unsafe_take_cstruct_ne 6, P.unsafe_take_cstruct_ne 5) <$$> fun a b ->
      Cstruct.append a b
    in
    Popper.(
      suite
        [
          ...
          ( "value is hello world",
            test (fun () ->
                let p = P.unsafe_take_cstruct_ne 20 in
                equal cstruct_result_comparator (P.run p inp)
                  (Ok (Cstruct.of_string "hello world"))) );
          ( "value is 'hello world'",
            test (fun () ->
          ...
          ( "value is 'hello world hello'",
            test (fun () ->
                let p =
                  ( P.take_cstruct 6,
                    P.unsafe_take_cstruct_ne 6,
                    P.unsafe_take_cstruct_ne 5 )
                  <$$$> fun a b c ->
                  let a = Cstruct.append a b in
                  Cstruct.append a c
                in
  ...
  let suites =
    Popper.suite
      [
        ...
        ("unsafe_take_cstruct_ne", unsafe_take_cstruct_ne);
      ]
end
