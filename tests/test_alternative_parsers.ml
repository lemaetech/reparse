  ...
  let any =
    ...
    Popper.(
      suite
        ...
        ; pos_test p2 5 inp; last_trimmed_pos_test p2 0 inp
        ; ( {|first success value is "he"|}
          , test (fun () ->
                equal string_result_comparator (P.run p3 inp) (Ok "he") ) )
        ; pos_test p3 2 inp; last_trimmed_pos_test p3 0 inp ])
