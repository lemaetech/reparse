...
let advance n state =
  let current_char offset =
    `Char
      ...
      | `Bigstring src -> Bigstringaf.unsafe_get src offset ) in
...
let parse src t =
  let len =
    ...
    | `Bigstring s -> Bigstringaf.length s in
  let state = {src; len; offset = -1; cc = `Eof} in
  R.bind (advance 1 state) (fun (state, ()) -> t state)
  |> function
  | Ok (_, a)    -> Ok a
  | Error (_, e) -> Error e
...
let count_skip_while f state =
  let rec loop count state =
    ...
    | Error (state, _) -> ok count state in
  loop 0 state
...
let take_while f state =
  let rec loop buf state =
    match satisfy f state with
    | Ok (state, c)    -> Buffer.add_char buf c ; loop buf state
    | Error (state, _) -> R.ok (state, Buffer.contents buf) in
...
let take_while_n n f state =
  let rec loop count buf state =
    ...
    else R.ok (state, Buffer.contents buf) in
